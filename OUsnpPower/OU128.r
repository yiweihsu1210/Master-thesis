...
num.repdata <- 100#200
seqlen<-1000
taxa.size.array <- c(128)  # c(50) 
sims<- 100

# ouparams<-c(5,5,40,40,90,80,100)

alp1 <- alp2 <-c(5, 10) 
sig1 <-sig2 <- c(1, 5, 10)  #c(1, 5, 10)  
th0 <- 90
th1 <-80
th2 <-100
...
# library(TreeSim)
# tree <- sim.bd.taxa(n=16, numbsim=1,lambda=100,mu=0.5)[[1]]
# plot(tree)
# vcv.tree<-vcv(tree)
...
for(alpIndex in 1:length(alp1)){
  ...
  for(sigIndex in 1:length(sig1)){
    ...
    print("power")
    print(power.bm.array[alpIndex,sigIndex,taxaIndex,treeIndex])
    print(power.ou.array[alpIndex,sigIndex,taxaIndex,treeIndex])
  }#end sig.value
}#end.alp.value
